...
`http://<your-LAN-IP>:5000`) — open that from your phone or another computer
on the same network.
...
`http://<your-LAN-IP>:5000` LAN address doesn't qualify, so from another
...
- **IP allowlist** (Settings → Security) — optional, off by default.
  ...
  (e.g. `<your-TAILNET-IP>/10` for Tailscale, `<your-LAN-IP>/24` for a LAN subnet) —
...
- **Activity log** — every add/change/remove (services, groups,
  ...
  `[2026-08-16T08:36:10Z] [service] Created "Jellyfin" (<your-TAILNET-IP>)`.
